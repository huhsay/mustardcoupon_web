...
<body>
    ...
    <table>
              ...
              <th scope="col" class="coupon">del</th>
    				</tr>
    			</thead>
    			<tbody>
    					<?php
              ...
              echo "$sd[name]"?></td>
    					<td class="stamp"><?php echo $data['count']?></td>
              <td class="coupon"><?php echo ($data['count']-($data['count']%10))/10?></td>
              <td class="delButton">
                <form class="button" action="http://168.131.35.106/main_user/delCoupon_process.php" method="post">
                  <input type="hidden" name="mSeq" value="<?php echo $memberSeq?>"/>
                  <input type="hidden" name="smSeq" value="<?php echo $storemb?>"/>
                  <input type="submit" value="쿠폰찢기">
                </form>
    				</tr>
          <?php endwhile ?>
